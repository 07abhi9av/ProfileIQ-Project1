    ...
    if choice == 'User':
        ...
        if st.button('Login'):
            if ad_user == 'abcd' and ad_password == 'abcd':
                ...
                # fig = px.pie(df, values=values, names=labels, title="Pie-Chart for Users Experienced Level")
                # st.plotly_chart(fig)

                labels = plot_data['User_level'].value_counts().index
                values = plot_data['User_level'].value_counts().values
                st.subheader("📈 Pie-Chart for User's Experienced Level")
                fig = px.pie(plot_data, values=values, names=labels, title="Pie-Chart for Users Experienced Level")
                st.plotly_chart(fig)
